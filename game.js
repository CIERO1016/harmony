...
function canPlace(type, stack) {
  ...
  switch (type) {
    ...
    // 建物：空きマスにも置ける（高さ1・得点0）。さらに山・木・建物(高さ1)の上に重ねると高さ2で得点。3段目は不可。
    case 'building': return h === 0 || (h === 1 && (top === 'mountain' || top === 'building' || top === 'tree'));
...
const PLACE_RULE = {
  ...
  building: '建物は空きマスに置けます（高さ1・得点0）。山・木・建物(高さ1)の上に重ねると高さ2で得点',
};
...
function showRules() {
  ...
  openModal(
    ...
    '<li><b>配置ルール</b>：全トークンは空きマスに置けます（隣接条件なし）。加えて重ね置き可：山=最大3段、木(幹)=最大2段、葉=幹の上、建物=山・木・建物(高さ1)の上（→高さ2で得点）。川/畑は平置きのみ。</li>' +
